...
from src.data_conversion.base import BaseArgs, BaseManager, COCOArgs, LabelmeArgs, validate_path
...
class LabelmeManager(BaseManager):
    ...
    def convert(self, target_format: str, data_cfg: BaseArgs, copy_images: bool = True):
        target_format = target_format.lower()
        self._validate_dataset()
        if target_format == 'coco':
            self._to_coco(data_cfg, copy_images=copy_images)
        else:
            raise ValueError(f"The target format [{target_format}] is not supported.")

    def _convert_to_coco(self, data_dir, target_dir, copy_images: bool = True) -> dict[str, Any]:
    ...
    def _get_img_info(self, img_id: int, json_file):
    ...
    def _get_annotation(self, img_id: int, img_data) -> None:
    ...
    def _to_coco(self, data_cfg: BaseArgs, copy_images: bool = False):
        def _convert_data(data_dir, target_dir, target_json):
            self.images.clear()
            self.categories.clear()
            self.annotations.clear()
        ...
        if self.args.is_img_same_dir:
            ...
            _convert_data(self.args.data_dir, data_cfg.data_dir, data_cfg.anno_data)
        else:
            if validate_path(self.args.train_dir):
                _convert_data(self.args.train_dir, data_cfg.train_dir, data_cfg.anno_train)
            if validate_path(self.args.val_dir):
                _convert_data(self.args.val_dir, data_cfg.val_dir, data_cfg.anno_val)
            if validate_path(self.args.test_dir):
                _convert_data(self.args.test_dir, data_cfg.test_dir, data_cfg.anno_test)

    def _check_images(self, img_dir) -> None:
