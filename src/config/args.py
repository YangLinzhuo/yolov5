...
class EnvironmentConfig(BaseConfig):
    server_id: int = data_field(default=0, metadata=meta(help_msg="Server id of current machine"))
    device_num: int = data_field(default=1, metadata=meta(help_msg="Number of device to be used on current machine"))
...
@dataclass
class BasicConfig(EnvironmentConfig):
...
@dataclass
class InferBasicConfig(BaseConfig):
...
class EvalConfig(BasicConfig, InferBasicConfig):
    distributed_eval: bool = data_field(default=False, metadata=meta(help_msg="Distribute test or not"))
...
class TrainConfig(EvalConfig):
    ...
    distributed_train: bool = data_field(default=False, metadata=meta(help_msg="Distribute train or not"))
...
class InferConfig(BasicConfig, InferBasicConfig):
    ...
    # Currently not support enabling rect for inference because of dynamic shape
